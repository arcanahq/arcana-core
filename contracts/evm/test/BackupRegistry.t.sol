...
contract BackupRegistryTest is Test {
    ...
    function test_SubmitBackup() public {
        ...
        (bytes32 latestId, string memory latestHash, uint256 latestTs) = backupRegistry.getLatestBackup(user1);
        assertEq(latestId, backupId);
        assertEq(latestHash, IPFS_HASH);
        assertEq(latestTs, timestamp);
    }
    ...
    function test_SubmitBackup_OnlyKeepsLatestPerUser() public {
        bytes32 backupId1 = keccak256("backup-1");
        bytes32 backupId2 = keccak256("backup-2");
        ...
        // Old backup ID should no longer resolve
        (string memory hash1, uint256 ts1) = backupRegistry.getBackup(user1, backupId1);
        assertEq(hash1, "");
        assertEq(ts1, 0);

        // Latest backup should resolve
        (string memory hash2,) = backupRegistry.getBackup(user1, backupId2);
        assertEq(hash2, IPFS_HASH2);

        (bytes32 latestId, string memory latestHash,) = backupRegistry.getLatestBackup(user1);
        assertEq(latestId, backupId2);
        assertEq(latestHash, IPFS_HASH2);

        // Registry now stores only one backup per user
    ...
    function test_SubmitBackup_Overwrite() public {
        ...
        vm.startPrank(user1);
        backupRegistry.submitBackup(backupId, IPFS_HASH);
        (,, uint256 timestamp1) = backupRegistry.getLatestBackup(user1);
        ...
        // Still only one backup stored per user
        uint256 count = backupRegistry.getUserBackupCount(user1);
        assertEq(count, 1);
    }
    ...
    function test_GetLatestBackup_NonExistent() public view {
        (bytes32 id, string memory hash, uint256 timestamp) = backupRegistry.getLatestBackup(user2);
