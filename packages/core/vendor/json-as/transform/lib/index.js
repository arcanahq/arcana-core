...
export class JSONTransform extends Visitor {
    ...
    addImports(node) {
        ...
        const isNodeModulesLib = path.dirname(baseDir).endsWith("node_modules");
        const isVendoredLib = baseDir.includes(path.join("vendor", "json-as"));
        const isLib = isNodeModulesLib || isVendoredLib;
        ...
        else if (isLib && !this.parser.sources.some((s) => s.normalizedPath.includes("json-as/assembly/index"))) {
            const libIndexPath = path.join(baseDir, "assembly", "index.ts");
            const newPath = path.posix.join(...libIndexPath.split(path.sep));
            this.parser.parseFile(readFileSync(libIndexPath).toString(), newPath, false);
        }
        ...
        if (isNodeModulesLib && baseRel.endsWith("json-as")) {
